...
- **Who closes to `done` depends on the workflow tier** (scope header `**Workflow:**` line; a feature's higher Weight tag wins; no scope → infer as in the recommendation step):
...
- **Recommend the next steps, scaled to the workflow depth.** Read the depth in this precedence: this feature's Weight tag if higher, else the project default on the scope header `**Workflow:**` line, else (no scope at all, e.g. a quick standalone `/develop`) infer it from the risk signals you already read (auth, payments, PII, data migrations, external money → heavier; a copy tweak, a styling change, a self contained component → `Vibe`/`Lean`). Then recommend only what that depth calls for, and surface the depth as an overridable statement, not a gate:
