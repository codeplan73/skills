...
4. **API surface** — the smallest surface that solves the problem. For each endpoint or function: name it, the HTTP method and path (or function signature), the 2–4 key request fields (name, type, required/optional), the key response fields, the authentication requirement (public / authenticated / role-restricted), and the 2–3 most important error cases (not exhaustive, only those that change how the caller must behave).
