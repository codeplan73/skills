...
2. **Open the governing ADR via the row's `ADR` pointer — read only what specs this sub-task.** A single ADR file → read it. An umbrella `index.md` → read the index (decision + child list), then open **the child ADR matching this sub-task**. It's the spec; proceed. Only if the row has **no** pointer *and* no ADR is linked, do a **targeted** look for one matching this feature's scope in its `docs/adr/<workspace>/` — never a blanket read of every ADR.
...
1. **The governing ADR — read only what specs *this* sub-task** (from the roadmap row's `ADR` pointer, or the one found in Step 0). **Single ADR file** → read it. **Umbrella `index.md`** (a directory decision) → read the `index.md` for the overall decision + its child list, then open **the child ADR whose scope matches this sub-task** and build from *that* child's detailed spec (one child, not all). Not the whole `docs/adr/` tree — just the spec for *this* work: data model, API surface, invariants, security model, the provider/library already chosen. **Check the `Status`** — on the single file, or on the umbrella `index.md` for a directory decision (child ADRs carry no lifecycle status): if it's still **`Proposed`** (not `Accepted`), the decision isn't ratified — warn before building: "The governing ADR is still `Proposed`, not accepted — build on an un-agreed decision, or accept it first (re-run `/architect` and confirm)?" Build only on the engineer's go-ahead. A `Superseded` ADR → use the one that superseded it.
