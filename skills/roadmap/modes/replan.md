...
The default cadence, not rare: run each time a feature or phase lands, keeping the roadmap matching reality and queueing the next slice. Reconciles in place, never spawns a new file; coarse and surgical (reconcile cells, append rows, don't rewrite the file).
...
3. Enroll needs surfaced during the build: read shipped features' ADR `## Consequences` and `## Follow-up` sections; a follow-up (e.g. "add rate limiting") not yet a roadmap row becomes a new `planned` row with intent, weight, `Needs ADR?`; the roadmap grows from real build feedback.
