...
<!-- Single decision: one file at that path. Directory ADR (umbrella, or a single decision with bulky research): write the top file always as `index.md` (never a doubled NNNN-title/NNNN-title.md), plus named child ADRs inside. When there are children or research, the top file includes a ## Structure manifest listing and linking EVERY child ADR and EVERY research file, one line each (what it is + which decision it supports), so the directory is fully mapped from one place. Each child ADR gets a ## References section linking ITS OWN research. ANY inventory/audit/research goes in the directory's research/ subfolder, named by its owner: research/NNNN-<topic>.md (the child's number) or research/_shared-<topic>.md for umbrella-wide evidence; NEVER in docs/roadmap/ (the roadmap), never loose in the code tree. Children are flat files by default; a child gets its own subfolder (NNNN-child/{index.md, research/}) only if it has multiple research/asset files. Keep each child self-sufficient to build from (research/ is the optional evidence trail, not required reading); put any cross-child contract (how children connect) in the top file. -->
**Operation**: OPERATION
...
- Use no dashes of any kind: no em dash, no en dash, no hyphen used as punctuation. Use short sentences, commas, or parentheses instead. (Hyphens inside real compound words and code, like `kebab-case` or `AC-1`, are fine; the rule is about dashes used as punctuation.) Clear beats clever.
...
- **Standalone decision ADR**: MODE is ARCHITECTURE or CROSS-CUTTING with no buildable roadmap feature tied to it. Also write **`Proposed`** at creation; ratification (not a build phase) promotes it to `Accepted`, and the main agent sets that on the engineer's confirmation.
- **Documenting already-shipped work**: DOCUMENTATION_CONTEXT is provided, OR the linked roadmap feature is already `existing` (shipped, pre-workflow). Write **`Accepted`**: the ADR describes reality that already exists (see the documentation-path rule below).
