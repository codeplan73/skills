...
**Task source — read the tasks from their home.** `/develop`'s task list is the feature's **roadmap row sub-tasks if that row exists**, else the ADR's **`## Build plan`** (the ordered tasks, each tagged "— satisfies AC-N", with the migration as task 1). Read from whichever is the home, and **tick completed tasks in that same home** (the roadmap checkbox, or the `## Build plan` line). A feature with no roadmap row still has a task list — the ADR's `## Build plan`.
...
- **Tick each task in its home.** If the feature has a roadmap row, tick its sub-task(s) there — **the one numbered file you located in Step 0** (the feature's file, in its workspace subdir for a monorepo; not the whole `docs/roadmap/` tree). If there's no roadmap row, tick the task in the ADR's **`## Build plan`** (`[ ]` → `[x]`). Either way tick only the task(s) you **verified** complete, fill in the feature's `Code area` (and `ADR`) pointers, and set the feature's **Status** to `done` only when every task is checked — otherwise leave it `in-progress`.
...
- **Emit verify steps, then offer to save them (every run).** At the end of *every* `/develop` run, derive **concrete verification steps from the ADR's acceptance criteria** — actionable and specific, each tied back to its `AC-N`, not vague advice. E.g. "visit `/signup` → sign up → expect redirect to `/auth/verify-email` → AC-1", "run `<migrate cmd>` → query confirms tables live → AC-4". Then present a decision panel (your agent's option picker — `AskUserQuestion` on Claude Code — or plain-text options with the same choices if it has none; single-select):
